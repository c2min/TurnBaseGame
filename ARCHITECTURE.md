...
- 2026-06-20 — 배틀 타깃 모델 = 2D TurnRPG 확정(ADR-005, 사용자). HSR 잔재 정리 방향. bridge REQUEST §3로 turnrpg-server·plan 공유. (코드 정리=후속)
...
| ADR-005 | 2026-06-20 | **배틀 타깃 모델 = 2D TurnRPG(= 서버) 확정. HSR 잔재 정리(supersedes ADR-004 잠정).** 발산 기원=클라가 2D TurnRPG(BD2/LastOrigin)→HSR/3D JRPG화→2D 회귀 미완(서버는 2D 유지) → 배틀 괴리(사용자 증언·확정 2026-06-20). 계통: 타일/그리드=2D 정본(유지) / SP·ATB 행동게이지·궁극기 게이지·멀티타겟·AoE=HSR 잔재(정리). | 기각=⒜HSR 유지+서버/계약 HSR 확장(서버 2D 도메인 대수술) ⒝하이브리드 방치. 트레이드오프=서버 권위·단일 모델 정합 ↔ 클라 HSR 기능 정리 비용(SP/궁극기/ATB/AoE 제거). | `[시점포착·사용자 확정]` `(memory turnrpg-battle-divergence-origin · REQUEST turnrpg-battle-snapshot-visualid §3)` |
