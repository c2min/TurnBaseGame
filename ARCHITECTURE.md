...
- 2026-06-20 — 배틀 타깃 모델 = 2D TurnRPG 확정(ADR-005, 사용자). HSR 잔재(SP·ATB게이지·궁극기 게이지) 정리·AoE는 유지. id 정본=int(ADR-006).
- 2026-06-20 `2151697` — 계약 재배포 소비: `BattleUnitDto += int TemplateId`(bridge `2b29b0f`, REQUEST visualid §1 done·additive). 아군/적 단일 키+Team 분기. 배틀-init 아군 비주얼 언블록. ⚠️ provisional 창 닫히기 시작(소비 개시)=이후 기존필드 reshape는 락스텝(bridge 인지).
- 2026-06-20 `171ff1a`/`5aa85c1`/`6bf28a6` — HSR 잔재 정리 3종: ATB 행동게이지→라운드 턴오더 · 스킬포인트 제거 · 궁극기 에너지 게이지 제거(궁극기 스킬은 일반스킬 보존). 서버 2D 라운드 모델 수렴.
- 2026-06-20 `0381acd` — 배틀-init 리워크: `BattleSnapshotPacket`→서버 권위 스폰(`SpawnFromSnapshot`·`BattleUnitServerData` 어댑터·TemplateId 비주얼·적0 graceful). 구 웨이브/PartyCache=오프라인 폴백.
- 2026-06-20 `cf7518b` — 배틀 그리드 단일 공유 모델 수렴(ADR-007). 팀별 5×3→가변 단일 그리드·글로벌 TileIndex. TileGroup/BattleFieldManager/Highlighter/View/StageDirector/SkillTargetResolver. `dotnet build 0err`.
...
| ADR-007 | 2026-06-20 | **배틀 그리드 = 단일 공유 가변 그리드(서버 정합). 클라 팀별 5×3 폐기.** turnrpg-server 규약 확정(HANDOFF battle-grid-tileindex-done): 아군·적 **한 그리드 공존**(FFT식)·글로벌 `TileIndex=Y*Width+X`·**스테이지별 가변 치수**(GridDto). 클라 BrownDust2식 팀별 5×3(`TileGroup` COL/ROW 정적·2그리드) → 단일 가변으로 수렴(사용자 확정). `TileGroup`(인스턴스 W/H)·`BattleFieldManager`(단일)·`TileHighlighter`(팀필터)·`BattleFieldView`(단일 동적 생성)·`StageDirector`(GridDto 치수). | 기각=⒜팀별 분리 그리드 유지(=서버 모델 변경, 단일 공유가 AoE/Area·이동 와이어와 정합) ⒝하이브리드. 트레이드오프=서버 권위·FFT 택틱스 정합 ↔ 클라 타일 UI 전면 리워크·레인 타게팅 의미 재정의(후속). | `[사용자 확정·서버 규약]` `(=turnrpg-server 확인 · HANDOFF battle-grid-tileindex-done: BattleGrid 단일·GridPosition.ToIndex=Y*w+X)` |
| ADR-006 | 2026-06-20 | **콘텐츠 id 정본 = numeric(int).** 템플릿/스테이지/스킬 id = int(plan+turnrpg-server 확정·데모 e2e). 클라 string("stage_001"·string SkillId)=드리프트 → int 정렬(현 `int.TryParse` 잠정→콘텐츠 데이터 int 전환). `CharacterId:long`=인스턴스 id(Phase1 stopgap `==TemplateId`, O7 DB 시 분리=server ADR-009). | 기각=클라 string 정본화(와이어 numeric reshape=락스텝·provisional 닫힘 불리). | `[사용자 도메인 외·확정]` `(=plan·turnrpg-server 확인, HANDOFF visualid §2)` |
| ADR-005 | 2026-06-20 | **배틀 타깃 모델 = 2D TurnRPG(= 서버) 확정. HSR 잔재 정리(supersedes ADR-004).** 발산 기원=클라가 2D TurnRPG(BD2/LastOrigin)→HSR/3D JRPG화→2D 회귀 미완(서버는 2D 유지)→괴리(사용자 확정 2026-06-20). **HSR 잔재(클라 정리)= SP·ATB 행동게이지·궁극기 에너지 게이지뿐**(자원/게이지 메카닉). **유지(2D 정본)= 타일/그리드·라운드 턴·그리드 AoE/Area/All 타게팅**. ⚠️ **AoE는 HSR 잔재 아님**(초기 오분류 정정, 전 세션 동의): 서버 `ETargeting{AreaEnemy/AllEnemies/AllAllies}+AoeRadius` 구현·데모 PASS=2D 택틱스 정본. 와이어=단일 앵커 `TargetUnitId`(서버가 AoE 해소) → 클라 *멀티타겟 송신*만 정리, AoE *능력* 유지. | 기각=⒜HSR 유지+서버 2D 도메인 HSR 확장(대수술) ⒝하이브리드 방치. 트레이드오프=서버 권위 단일모델 ↔ 클라 SP/ATB/궁극 게이지 정리 비용. | `[시점포착·사용자 확정·전세션 정합]` `(memory turnrpg-battle-divergence-origin · HANDOFF visualid §3 bridge·turnrpg-server·plan ack)` |
| ADR-004 | 2026-06-20 | **배틀 모델 발산 = 계약/서버 모델로 수렴, 구 클라 발산기능은 클라 로컬 강등(잠정).** ⤳ ADR-005가 방향 확정(2D·HSR정리). 구 클라(ATB 행동게이지·스킬포인트·웨이브·멀티타겟/AoE) ↔ 계약·turnrpg-server(라운드기반 NextUnitId/RoundNumber·그리드+유닛 스냅샷·단일타겟·SP 없음). charter 원칙(클라뷰=서버권위 상태의 표현) 적용 → SP=클라 로컬 낙관·ATB SyncTurnOrder 생략(턴루프=서버 NextUnitId)·멀티타겟→첫타겟·SkillId/StageId string→int 파싱. | 기각=⒜구 클라 모델 유지(서버 미지원 기능=와이어 부재로 동작 불가) ⒝계약 확장 즉시 REQUEST(설계 미확정 상태 선행 변경). 트레이드오프=서버 권위 정합·즉시 컴파일/진행 ↔ 발산기능 강등(기능 손실 표면). **잠정** — turnrpg 정식 설계(SP/ATB/웨이브/AoE 유무)=turnrpg-server·plan SoT, 확정 시 계약 확장 REQUEST 또는 클라 정리로 재결정. **배틀-init(스냅샷 그리드모델)은 미배선**: `BattleUnitDto`가 UnitId만 운반(템플릿/비주얼 id 부재)→비주얼 해소 불가, 그리드 init 리워크 보류. | `[시점포착]` `(grep BattleSkillUseRequest·InGameSceneController·UISkillBar · BattleSnapshotPacket{Grid,Units})` |
